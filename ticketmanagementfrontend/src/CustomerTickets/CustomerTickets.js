...
const CustomerTickets = ({ tickets }) => {
  ...
  const getPriorityColor = (priority) => {
    switch (priority.toLowerCase()) {
      ...
      default:
        return "#FFFFFF"; // White for unknown priority (Fallback)
    }
  };
  ...
  const handleTicketClick = (ticketId) => {
    navigate(`/customermain/CustomerTicketUpdate/${ticketId}`);
  };
  ...
  return (
    <div className="p-4 mb-10">
      ...
      {filteredTickets.map((ticket) => (
        ...
        >
          <div className="text-[aliceblue]">{ticket.ticketCode}</div>
          ...
          <div className="truncate-description text-[aliceblue]">
            {truncateDescription(ticket.explainproblem, maxDescriptionLength)}
          </div>
        </motion.div>
      ))}
    </div>
